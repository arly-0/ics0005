...
public class DoubleStackTest {
   ...
   @Test(timeout = 1000)
   public void testInterpretWithSwap() {
      assertEquals("2. 5. SWAP - should result in 3.", 3., DoubleStack.interpret("2. 5. SWAP -"), delta);
   }

   @Test(timeout = 1000)
   public void testInterpretWithDup() {
      assertEquals("3. DUP * should give 9.", 9., DoubleStack.interpret("3. DUP *"), delta);
   }

   @Test(timeout = 1000)
   public void testInterpretWithRot() {
      assertEquals("2. 5. 9. ROT - - should give -2.", -2., DoubleStack.interpret("2. 5. 9. ROT - -"), delta);
   }

   @Test(timeout = 1000)
   public void testInterpretComplexExpression() {
      assertEquals("-3. -5. -7. ROT - SWAP DUP * + should give 21.", 21.,
            DoubleStack.interpret("-3. -5. -7. ROT - SWAP DUP * +"), delta);
   }

   @Test(timeout = 1000, expected = IllegalArgumentException.class)
   public void testUnderflowWithDup() {
      DoubleStack.interpret("DUP");
   }

   @Test(timeout = 1000, expected = IllegalArgumentException.class)
   public void testUnderflowWithSwap() {
      DoubleStack.interpret("1. SWAP");
   }

   @Test(timeout = 1000, expected = IllegalArgumentException.class)
   public void testUnderflowWithRot() {
      DoubleStack.interpret("1. 2. ROT");
   }
}
